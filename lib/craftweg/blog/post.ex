defmodule Craftweg.Blog.Post do
  ...
  defstruct [:path, :slug, :title, :excerpt, :date, :tags]

  def build(path, _attrs, body) do
    filename_without_extension = path |> Path.rootname() |> Path.split() |> Enum.take(-1)
    [year, month, day] = filename_without_extension |> String.split("-", take: 3)
